document.addEventListener("DOMContentLoaded", () => {
  ...
  if (pricingEl) {
    pricingEl.innerHTML = `<strong>${Mouneh.formatPrice(product.price)}</strong>${product.oldPrice ? `<span class='old-price'>${Mouneh.formatPrice(product.oldPrice)}</span>` : ""}`;
  }

  if (availabilityEl) {
    availabilityEl.textContent = product.stock > 0 ? `In stock (${product.stock} left)` : "Out of stock";
  }

  function renderGallery(selectedImage) {
    if (!galleryMain || !galleryThumbs) return;
    galleryMain.style.backgroundImage = `url('${selectedImage}')`;
    galleryThumbs.innerHTML = product.images
      .map(
        (src, index) => `
          <button class="gallery-thumb" type="button" data-src="${src}" aria-label="View image ${index + 1}" style="background-image:url('${src}')"></button>`
      )
      .join("");
